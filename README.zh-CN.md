...
<!-- asciinema 官方嵌入：点击封面在 asciinema.org 上播放 -->
...
| 特性 | fspack | PyInstaller | Nuitka | cx_Freeze | Briefcase |
|------|--------|-------------|--------|-----------|-----------|
| 一行命令打包 | ✅ `fsp b` | `pyinstaller --onefile` | `python -m nuitka` | `cxfreeze` | `briefcase build` |
| 自动依赖推断 | AST 闭包 | 钩子+隐式导入 | AST + 运行时跟踪 | 配置文件 | 手动声明 |
| 智能精简 | 按 import 闭包 | 基础剥离 | 无 | 无 | 无 |
| 跨平台安装包 | Win/Linux/macOS | 仅可执行文件 | 仅可执行文件 | 仅可执行文件 | Win/macOS |
| Win7 兼容 | ✅ API shim | ❌ | ❌ | ❌ | ❌ |
| 离线打包 | ✅ | ❌ | ❌ | ❌ | ❌ |
| 国内镜像 | 清华/阿里 | 需手动 | 需手动 | 需手动 | ❌ |
| 多入口 monorepo | ✅ `-R` | 需多命令 | 需多命令 | 需多命令 | ❌ |
| Nuitka 加速 | `--nuitka` | ❌ | 强制 Nuitka | ❌ | ❌ |

...
def test_build_dry_run_skips_runtime_download(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("fspack.packaging.pipeline.stages.download_embed", fake_download)
...
def test_build_dry_run_skips_linux_runtime_download(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("fspack.packaging.pipeline.stages.download_standalone", fake_download)
    monkeypatch.setattr("fspack.packaging.pipeline.stages.extract_standalone", fake_extract)
    monkeypatch.setattr("fspack.packaging.pipeline.stages.download_wheels", lambda *a, **k: [])
    monkeypatch.setattr("fspack.packaging.pipeline.stages.unpack_wheels", lambda *a, **k: 0)
    monkeypatch.setattr(
        "fspack.packaging.pipeline.stages.compile_loader",
...
def test_build_dry_run_returns_project_info(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    shutil.copytree(_EXAMPLES / "gui" / "tk_app", proj)

...
def project_templates_dir() -> Path:
    ...
    位于 fspack 包内的 ``assets/templates/``，打包后随 wheel 分发。
    """
...
def _parse_template(dir_path: Path) -> ProjectTemplate | None:
    ...
    if not pyproject.is_file():
        _logger.warning("跳过无 pyproject.toml 的目录: %s", dir_path.name)
        return None

    try:
        data = tomllib.loads(pyproject.read_text(encoding="utf-8"))
    except (OSError, ValueError, tomllib.TOMLDecodeError) as e:
        _logger.warning("解析 %s/pyproject.toml 失败: %s", dir_path.name, e)
        return None
    ...
    return ProjectTemplate(
        dir=dir_path,
        id=dir_path.name,
        name=proj.get("name", dir_path.name),
...
def get_project_template(template_id: str) -> ProjectTemplate | None:
    ...
    dir_path = project_templates_dir() / template_id
    if not dir_path.is_dir():
        return None
    return _parse_template(dir_path)

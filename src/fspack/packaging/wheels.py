"""fspack wheel 下载 facade：从 wheel_pip/wheel_cache/wheel_markers 三个模块 re-export 公开 API.

本模块是 :mod:`fspack.packaging.wheels` 的入口与 API 索引，无业务逻辑。原
``wheels.py``（709 行）按职责拆分到三个模块：

- :mod:`fspack.packaging.wheel_pip`：pip/uv 调用 + sdist 回退 + 流式输出 +
  ``download_wheels`` 入口 + wheel 文件名解析
- :mod:`fspack.packaging.wheel_cache`：依赖解析缓存（``.deps-<key>.json``）
- :mod:`fspack.packaging.wheel_markers`：``python_version`` 环境标记预过滤
...
对 :mod:`fspack.packaging.wheel_pip` 内的调用同样有效。
"""

from __future__ import annotations

# 显式导入标准库模块：兼容测试中 ``fspack.packaging.wheels.<module>.<attr>`` 的 patch 路径。
# 这些模块为单例，patch 设置属性后对 wheel_pip/wheel_cache/wheel_markers 同样生效。
import os  # noqa: F401
import re  # noqa: F401
...
from fspack.packaging.wheel_cache import (
    _deps_cache_key,  # noqa: F401
    _load_deps_cache,  # noqa: F401
    _save_deps_cache,  # noqa: F401
)
from fspack.packaging.wheel_markers import (
...
from fspack.packaging.wheel_pip import (

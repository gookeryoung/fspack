...
本模块是 facade，通过多继承组合三个职责单一的 mixin：

- :class:`fspack.packaging.nuitka_env.NuitkaEnv` — 环境就绪
  （C 编译器检查、standalone python 准备、nuitka 安装、ccache）
- :class:`fspack.packaging.nuitka_compile.NuitkaCompile` — 编译流程
  （单文件 ``--module`` 编译、产物剥离、stamp 缓存、第三方包编译）
...
# facade 显式 import 这些模块（patch 设置的是模块对象的属性，全局生效，对 env/compile/verify
# 三个 mixin 模块同样有效）。详见 rule-01 公开 API 不变约束。
...
class NuitkaCompiler(NuitkaEnv, NuitkaCompile, NuitkaVerify):
    """Nuitka 编译器 facade（多继承自 env/compile/verify 三个 mixin）.
